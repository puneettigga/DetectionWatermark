...
class WatermarkAnalyzer {
	...
	public intersection(imgArr:any): any {
		...
		let wm_bb = this.extract_BoundBox(wm_mat);
...
function start(path: string): any {
	...
	let bb = wm.extract_BoundBox(wm_img);
	let img1 = bb[0];

	//cv.imwrite(path + "cont_grad.png", img1);

	let ofpath = Path.join(process.argv[2], 'bg_info.js');
	if (bb[bb.length-1] > 0) {
		bb = wm.intersection(imgArr);
		let img = bb[0];
		//cv.imwrite(path + 'bb_inter.png', img);
